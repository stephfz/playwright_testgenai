---
mode: agent
tools: ['codebase', 'problems', 'changes', 'testFailure', 'terminalSelection', 'terminalLastCommand', 'fetch', 'findTestFiles', 'searchResults', 'editFiles', 'search', 'runCommands', 'runTasks', 'playwright']
...
Generate a .md file with the report in the `manual-tests` directory and include any relevant screenshots or snapshots, add prefix to the filename with time, date and website name so that it is unique.
